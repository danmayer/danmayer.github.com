...
If you want to drive performance improvements for a library, you will need to understand some of the use cases and the structure of the code. We have covered how to use the Rails cache and we took a deep look at the [caching code workflow](/ruby/2024/10/24/caches-rails-workflows) including where time is most often spent, as well as digging into [cache network requests](/ruby/2024/11/07/caches-network-requests) to see where network IO time is spent. Now if you want to start to improve caching you can look at adding features to help avoid issues like cache stampede or you can focus on reducing the latency for using the cache. In this post we will take a look at reducing latency when using the cache.

# Understanding latency with benchmarks and profiles

If we want to make something faster we want data to help drive the improvements, often the best way to speed something up is using by using benchmarks and profilers. I find when I reach for one of these tools I often want both. I can use the benchmarks to see where I am in a range of options and conditions, then use the profiler to dig into specific use cases that don't match my needs or expectations to target very specific improvements. After fixing the profile, the increased performance is usually obvious in the relavent benchmarks as well. This can be a great feedback loop for driving improvements. It can also be helpful at avoiding regressions. Which is why on many of the projects I work on my team and I end up setting up both benchmarks and profiling as part of the CI pipeline. We have a full history of performance improvements, and we can fail CI for significant regressions, or at least investigate the regression which is sometimes required for a bug fix or new feature.

# Benchmarking Dalli with Github Actions

Let's take a look at how we can setup a suite of benchmarks for the Dalli Rubygem which powers the memcached flavor of `Rails.cache`.
...
This add a new ation which will install everything needed, run two instances of memcached and then run our benchmark script.
...
Similar to the benchmark this installs the requires things and gets the memcached server up and running then kicks off our profile script. The action then uploads all of the profiling results which can be retrieved after any CI run.
...
It depends on exactly your needs, but let's say we want to target making an improvement in writing to the cache, perhaps we have noticed that we have some write heavy workloads that are not performing as well with Dalli as other cache implementations. Beyond having the benchmarks as part of CI we have several applications the include benchmarking scripts so we can compare benchmarks across various caching backends, which helps us spot unexpected differences.
...
* some more smaller wins avoiding some other string manipulation 

After each code change you cna look at the new profile flame graph and see if it fixed the issue you were targetting.

![flamegraph after fixing](/assets/img/dalli_new_set.jpg)

Then finally run your benchmarks again to see if fixing what you saw in the profile data at least shows up as an improvement in your microbenchmark.
...
When you have fixed all the issues in the profile, you rerun the benchmark and hopefully the improvement causes a noticible difference in your benchmark. While things are different in real world applications, often what shows up in your microbenchmark will translate into real world improvements.

# Ship your performance Improvement

You can now ship a PR that includes the before and after benchmark data, as well as include screen shots showing the improvement in the flame graph.

This basic process was followed for a real [PR improving the set performance for dalli](https://github.com/Shopify/dalli/pull/39), which you can check out for full details.
...
# Invest in the toolchain
 ...
 __NOTE:__ At the moment these improvements are on a research fork of Dalli, but we have talked with the Dalli maintainer and will start to port various improvements into the released gem.
